...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        int maxLength = (dataLength - _outputBufferOffset > 8192) ? 8192 : (dataLength - _outputBufferOffset);
        NSInteger bytesWritten = [_outputStream write:_outputBuffer.bytes + _outputBufferOffset maxLength:maxLength];
        if (bytesWritten == -1) {
        ...
        if ([self.delegate respondsToSelector:@selector(webSocket:byteTraffic:)]) {
            [self.delegate webSocket:self byteTraffic:bytesWritten];
        };
        ...
        switch (eventCode) {
            ...
            case NSStreamEventHasBytesAvailable: {
                ...
                while (_inputStream.hasBytesAvailable) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(webSocket:byteTraffic:)]) {
                        [self.delegate webSocket:self byteTraffic:bytes_read];
                    };
